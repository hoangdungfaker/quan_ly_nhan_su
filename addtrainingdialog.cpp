...
AddTrainingDialog::AddTrainingDialog(QWidget *parent) : QDialog(parent)
{
    setWindowTitle("Them Khoa Hoc Dao Tao");
    ...
    maKhoaHocEdit = new QLineEdit(this);
    tenKhoaHocEdit = new QLineEdit(this);
    ...
    donViDaoTaoEdit = new QLineEdit(this);

    formLayout->addWidget(new QLabel("Ma Khoa Hoc:"), 0, 0, Qt::AlignLeft);
    formLayout->addWidget(maKhoaHocEdit, 0, 1);
    formLayout->addWidget(new QLabel("Ten Khoa Hoc:"), 1, 0, Qt::AlignLeft);
    formLayout->addWidget(tenKhoaHocEdit, 1, 1);
    ...
    formLayout->addWidget(new QLabel("Don Vi Dao Tao:"), 4, 0, Qt::AlignLeft);
    formLayout->addWidget(donViDaoTaoEdit, 4, 1);
    ...
    connect(buttonBox, &QDialogButtonBox::accepted, this, &AddTrainingDialog::onAccept);
...
bool AddTrainingDialog::isDateAfter(const QDate& date1, const QDate& date2) {
    return date1 > date2;
}

void AddTrainingDialog::onAccept()
{
...
DaoTao AddTrainingDialog::getTraining() const
{
    DaoTao dt;
    dt.setMaKhoaHoc(trim(maKhoaHocEdit->text().toStdString()));
    dt.setTenKhoaHoc(trim(tenKhoaHocEdit->text().toStdString()));
    dt.setNgayBatDau(ngayBatDauEdit->date().toString("dd/MM/yyyy").toStdString());
    dt.setNgayKetThuc(ngayKetThucEdit->date().toString("dd/MM/yyyy").toStdString());
    dt.setDonViDaoTao(trim(donViDaoTaoEdit->text().toStdString()));
    return dt;
}
